...
### 2. 深度解析报告 (The Report)
按以下 Markdown 结构在内存中生成完整分析结果，**不要**直接在对话框输出全部内容：
...
### 3. 持久化存储 (Persistence)
- **文件名提取**：从文章中提取最核心的标题，将其转换为合法的文件名（例如：`深度解析-文章标题.md`）。
- **执行写入**：调用 `write_to_file` 工具，将生成的完整报告写入到当前工作目录下的该文件中。
- **最终反馈**：在对话框中告知用户文件已生成，并给出报告中的 **“一句话核心 (TL;DR)”** 和 **“带着问题去读”** 部分，引导用户打开文件。

## 交互示例
> User: "帮我看看这篇关于 Serverless 的文章 [URL]"
> You: (读取内容 -> 在内存中生成报告 -> 调用 `write_to_file` 保存为 `深度解析-Serverless的未来.md` -> 告知用户文件已保存并展示摘要)
